--
-- This file contains the table definitions.
-- 
...
CREATE TABLE Houses
(
    uuid            CHAR(36)            PRIMARY KEY NOT NULL           , -- ATTR:AUTO_ON_C(UUID), NO_U(UUID); 32 characters; allocate 36 characters for terminating '\0'
    cusr            CHAR(52)            NOT NULL                       , -- creation user
    state_          TINYINT                                            , -- ? 什么状态，取什么值
    cdate           DATETIME                                           , -- ATTR:AUTO_ON_C(DATETIME), NO_U(DATETIME); creation date
...
(
    uuid            CHAR(36)            PRIMARY KEY NOT NULL           , -- ATTR:AUTO_ON_C(UUID), NO_U(UUID);
    cusr            CHAR(52)            NOT NULL                       , -- creation user
    state_          TINYINT(1)                                         , -- ? 什么状态，取什么值
    cdate           DATETIME                                           , -- ATTR:AUTO_ON_C(DATETIME), NO_U(DATETIME);
    ...
    photopath       TEXT(4000)                                         , -- 身份证照片路径(需要加密)
    ...
    patrol          TINYINT(1)                                         , -- 巡访
    finger1         TEXT(4000)                                         , -- 指纹采集一 (需要加密)
    finger2         TEXT(4000)                                         , -- 指纹采集二 (需要加密)
...
(
    uuid            CHAR(36)          NOT NULL                         , -- ATTR:AUTO_ON_C(UUID), NO_U(UUID);
    dev_number      CHAR(36)          NOT NULL                         , -- Added by DUSUN. 设备编号
    name            CHAR(64)                                           , -- 设备名称
...
CREATE TABLE Cards
(
    uuid            CHAR(36)          PRIMARY KEY NOT NULL             , -- ATTR:AUTO_ON_C(UUID), NO_U(UUID); 主键（关联表中外键引用）
    ...
    cdate           DATETIME          NOT NULL                         , -- ATTR:AUTO_ON_C(DATETIME), NO_U(DATETIME);
...
(
    ...
    cdate           DATE              NOT NULL                         , -- ATTR:AUTO_ON_C(DATE), NO_U(DATE);
    udate           DATE              NOT NULL                         , -- ATTR:AUTO_ON_C(DATE), AUTO_ON_U(DATE);
...
(
    ...
    parm2           VARCHAR(52)                                        , 
    edate           DATE                                               , -- 关联结束日期
    state_          CHAR(16)                                           , -- Added as per protocol requirement
    ...
    sync            TINYINT                                              -- ATTR:AUTO_ON_C(SYNC); 0 未同步; 1 已同步到公共平台
    -- FOREIGN KEY (crk_uuid) REFERENCES Cards(uuid)
    -- FOREIGN KEY (userid) REFERENCES Persons(uuid)
    -- FOREIGN KEY (houseid) REFERENCES Houses(uuid)
);
...
(
    ...
    dev_number      CHAR(36)                                           , -- 设备编号
    area_code       CHAR(64)          NOT NULL                         , -- 区域 层级编码
    dev_date        DATETIME                                           , -- Added as per protocol requirement
    flag            TINYINT           NOT NULL                         , -- ATTR:AUTO_ON_C(FLAG), AUTO_ON_U(FLAG); flag 导入类型：增｜删｜改，C｜D｜U
    sync            TINYINT                                              -- ATTR:AUTO_ON_C(SYNC); 0 未同步; 1 已同步到公共平台
    -- FOREIGN KEY (person_uuid) REFERENCES Persons(person_uuid), or CardOwnings(person_uuid)
    -- FOREIGN KEY (mac) REFERENCES Devices(mac)
    -- FOREIGN KEY (area_uuid) REFERENCES Devices(area_uuid)
    -- FOREIGN KEY (dev_number) REFERENCES Devices(dev_number)
    -- FOREIGN KEY (area_code) REFERENCES Houses(code), or Cards(layercode)
);
...
(
    ...
    cdate           DATETIME          NOT NULL                         , -- ATTR:AUTO_ON_C(DATETIME), NO_U(DATETIME); 插入本条记录的时间
...
(
    ...
    signal_         FLOAT                                              , -- 信号
    udate           DATETIME          NOT NULL                         , -- ATTR:AUTO_ON_C(DATETIME), AUTO_ON_U(DATETIME); 更新时间
    cardpopedomcapacity INT                                            , -- ? 单位? 卡容量
    cardpopedomcount INT                                               , -- 门锁白名单数量
    fingercapacity  INT                                                , -- 指纹容量
    fingercount     INT                                                , -- 指纹数
    opened          TINYINT                                            , -- 门开关状态 0: 门闭合 1: 门打开
    cdate           DATETIME          NOT NULL                         , -- ATTR:AUTO_ON_C(DATETIME), NO_U(DATETIME); 创建时间
