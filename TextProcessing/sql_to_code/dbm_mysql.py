...
def gen_mysql_construct(entity, file_out):
    ...
    print("".ljust(4) + 'Char *{pValue} = NULL;'.format(pValue = pValue), file = file_out, end = '\n');
    
    for i in range(0, count):
        field = entity.fields[i];
        dest = "p{entity_name}->{field_name}".format(entity_name = entity.singular_name, field_name = field.name);
        src = "(Char *) (*pRow)[{counter}]".format(counter = i)        
        print("".ljust(4) + '{pValue} = {src};'.format(pValue = pValue, src = src).ljust(31), file = file_out, end = '');
        print('if (!OSA_strIsEmpty({src})) '.format(src = pValue).ljust(28), file = file_out, end = '');
